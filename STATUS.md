...
- Interactive ticket table: agent summary responses render as a scrollable `ft.DataTable` with checkboxes; selected ticket keys accumulate in `app_state["selected_tickets"]`; "Process Selected" button shows current selection via SnackBar; KEY column cells are `ft.TextButton` links that open the Jira ticket URL in the browser; SUMMARY column enforces 400 px width via `ft.Container`; BLOCKER column enforces 150 px width; table has BLUE_GREY_800 border on all sides with selection row highlight
