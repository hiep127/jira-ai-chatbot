...
| File | Status | Notes |
|---|---|---|
| `backend/main.py` | ✅ | FastAPI app, `/ping`, `/health`, `/chat`, `/compact`; `GET /auth/github/status`, `POST /auth/github/spawn-terminal`; `GET /models` (fetches Copilot model list, `ModelInfo` schema, caches in `app.state.models_cache`, busts on `?refresh=true`); `model_id` field on `ChatRequest` and `CompactRequest`; 401 guard in `/chat` requires valid GitHub CLI token; mock MCP fallback removed — live `jira-harness` server only, fail-fast on connection error; `POST /reload-profiles` endpoint hot-reloads MCP subprocess with updated credentials without app restart; `_build_mcp_env()` reads `get_profiles()` + keyring → builds `JIRA_PROFILES_JSON` env var; `asyncio.Lock` prevents concurrent reload races; `/compact` endpoint now uses `await call_copilot(summary_prompt, model_id)` instead of `build_llm` + `ainvoke`; new `GET /auth/copilot/status` diagnostic endpoint returns `CopilotStatusResponse` (`gh_authenticated`, `copilot_ok`, `error_status`, `error_detail`); `ChatResponse` has new `tickets: list[dict] = []` field; `/chat` handler extracts `result.get("ticket_table_data", [])` and includes it in the response; **new `POST /chat/stream` SSE endpoint** (`StreamingResponse`, `media_type="text/event-stream"`) — consumes `graph.astream(stream_mode="updates")`, emits `progress` events per node + final `result` event with `response`/`tickets`/`thread_id` + `error` event on exception; same auth guard as `/chat`; existing `/chat` endpoint unchanged |
| `backend/agent/state.py` | ✅ | `AgentState` with `ticket_summaries: Annotated[list[str], operator.add]` reducer; `TicketState` with `ticket_id` field for map-reduce fan-out; `ticket_table_data: list[dict]` field added — written once by `aggregate_summary_node`, read by `/chat` endpoint |
| `backend/agent/nodes.py` | ✅ | Deleted 3 dead legacy functions (`make_summarizer_node`, `route_after_summarizer`, `extract_summary_node`); `make_llm_node` and `make_ticket_summarizer_node` converted to `async def`; MCP `fetch_tool.invoke` wrapped in `asyncio.to_thread`; LLM calls replaced with `await call_copilot()`; `ToolMessage` import removed; `Command` added to `langgraph.types` import; all return paths in `make_discovery_and_dispatch_node` now return `Command` — 3 error paths use `Command(update=..., goto="aggregate_summary_node")`, fan-out uses `Command(goto=[Send(...)])`; `make_aggregate_and_report_node` now also returns `ticket_table_data: list[dict]` (one dict per ticket row with keys `key`, `instance`, `status`, `summary`, `blocker`, `updated`, `aging`, `url`); `url` derived from `lge_base`/`spaws_base` env vars using `_LGE_PREFIXES` routing — empty string when env vars not set; **REQ-001 fix:** happy-path fan-out `Command` now includes `update={"tickets": [{"key": k} for k in valid_keys]}` so `astream(stream_mode="updates")` emits a chunk for `discovery_and_dispatch_node` that the SSE endpoint can forward as a "Fetched N tickets" progress event |
| `backend/agent/graph.py` | ✅ | Flat map-reduce graph: nodes `ticket_summarizer_node` + `aggregate_summary_node`; all `summarizer_daily` / `aggregate_and_report` references replaced; dead `route_after_dispatch` function + `add_conditional_edges` call removed — routing from `discovery_and_dispatch` is now handled entirely via `Command.goto` inside the node |
...
| File | Status | Notes |
|---|---|---|
| `frontend/main.py` | ✅ | Flet desktop app; starts backend in-process (thread); 10 s health-check timeout; auth guard container (`refresh_auth_state()`); `app_state` carries `model_id`/`model_name`/`model_tier`/`selected_tickets`/`models_cache`; `model_id` forwarded in `/chat` and `/compact` payloads; model chip button opens `open_model_picker`; GitHub Copilot 401 → `page.show_dialog(SnackBar)`; `FilterChip` active-profile chips rendered in **left sidebar** via `rebuild_sidebar()` (Active Profiles section above Active Query/JQL section); standalone `profile_chips_row` variable removed; `_build_profile_chips`, `_toggle_profile`, `_profiles` defined before `load_filter_settings()` to eliminate forward-reference; `_on_settings_saved()` refreshes `_profiles` in-place then calls `rebuild_sidebar()` (no redundant chip-row update); active profiles persisted to `settings.json` and restored on launch; `prefixes` in `/chat` payload set from active profiles; `on_daily_summary` derives effective JQL from active profile `custom_jql` fields when `app_state["custom_jql"]` is empty (single profile → direct JQL; multiple → OR-joined) — fixes false-positive "No JQL configured" error; **REQ-001:** `process_chat_message()` rewritten to consume `/chat/stream` SSE via `httpx AsyncClient(timeout=None)`; `progress` events appended as italic `GREY_500` size-12 rows inside a `BLUE_GREY_900` progress container (stays visible after result); `result` event renders ticket table or assistant bubble; `error` event shows error dialog; controls re-enabled in `finally` block; **REQ-004:** `_prefetch_models()` replaced by `_preload_models()` that stores result into `app_state["models_cache"]`; triggered inside `refresh_auth_state()` when authenticated; `cached_models=app_state.get("models_cache")` passed to every `open_model_picker` call |
| `frontend/components/__init__.py` | ✅ | Empty package marker for `frontend/components/` |
| `frontend/components/model_picker.py` | ✅ | `open_model_picker(page, app_state, on_model_selected, cached_models=None)` — `AlertDialog` (360 px); **REQ-003:** X close button added to title via `ft.Row([ft.Text(expand=True), ft.IconButton(ft.Icons.CLOSE)])`, calls `page.pop_dialog()`; search field + Refresh + Gear header; `ft.ListView` of `ListTile` rows; **REQ-004:** `cached_models` param — if provided, renders immediately without spinner; Refresh updates both `_state["models"]` and `app_state["models_cache"]`; **REQ-005:** active model pinned to top with `BLUE_GREY_700` background highlight and bold name; divider separates pinned row from remaining list; `_build_row(model, is_active)` takes explicit `is_active` param instead of re-checking `app_state`; collapsible "Other Models" section; `ft.ProgressRing` while fetching; 401 → closes picker and opens `open_config_dialog`; writes `model_id`/`model_name`/`model_tier` into `app_state` |
| `frontend/components/ticket_table.py` | ✅ | `build_ticket_table(tickets, app_state, page) -> ft.Container` — renders agent ticket results as a native `ft.DataTable` with **8 columns** (✓, KEY, INSTANCE, STATUS, SUMMARY, BLOCKER, LAST UPDATED, AGING); horizontal scroll via `ft.Row(scroll=AUTO)`; **REQ-002:** `show_checkbox_column=True` removed (non-functional in Flet 0.84); manual `ft.Checkbox` added as first `DataCell` per row with matching `DataColumn("✓")` header; `on_change` wires checkbox to `app_state["selected_tickets"]` and syncs `row.selected` for highlight; `row.on_select_changed` removed to avoid double-toggle; "Process Selected" `ElevatedButton` reads selection at click time and shows `SnackBar`; KEY cell renders as `ft.TextButton` with `page.launch_url(url)` on click (guard: skipped when url is empty); SUMMARY cell wrapped in `ft.Container(width=400)` to enforce column width; BLOCKER cell wrapped in `ft.Container(width=150)`; table has `border=ft.Border(all 4 sides, BLUE_GREY_800)` and `data_row_color={SELECTED: BLUE_GREY_800}`; `_make_row` accepts `page: ft.Page` as third param; Flet 0.84 compliant (`ft.Icons.*`, `ft.Colors.*`, `ft.Padding()`, `page.show_dialog()`) |
...
- Model chip button (header) → opens model picker; selected model name shown on chip; active model pinned to top with blue highlight; X button closes dialog without clearing selection; model list pre-loaded from app start (no spinner on first open)
...
- **Streaming progress display:** while agent runs, a `BLUE_GREY_900` progress container renders live italic grey rows — "Fetched N ticket(s)…", "Summarising KEY-X…", "Compiling summary…" — stays visible after the final result
- Interactive ticket table: agent summary responses render as a scrollable `ft.DataTable` with 8 columns; first column is a manual `ft.Checkbox` (✓ header); selected ticket keys accumulate in `app_state["selected_tickets"]`; "Process Selected" button shows current selection via SnackBar; KEY column cells are `ft.TextButton` links that open the Jira ticket URL in the browser; SUMMARY column enforces 400 px width via `ft.Container`; BLOCKER column enforces 150 px width; table has BLUE_GREY_800 border on all sides with selection row highlight
...
| `INVALID_GRAPH_NODE_RETURN_VALUE` — `Expected dict, got [Send(...)]` | LangGraph 1.x no longer accepts bare `list[Send]` from a node; `make_discovery_and_dispatch_node` now returns `Command` on all paths — fan-out via `Command(goto=[Send(...)])`, error paths via `Command(update=..., goto="aggregate_summary_node")`; dead `route_after_dispatch` and `add_conditional_edges` removed from `graph.py` |
| `show_checkbox_column=True` renders no checkbox column in Flet 0.84 | `ft.DataTable(show_checkbox_column=True)` is silently non-functional; replaced with manual `ft.Checkbox` as first `DataCell` per row, matching `DataColumn("✓")` header; `on_change` syncs `app_state["selected_tickets"]` and `row.selected`; `row.on_select_changed` removed to avoid double-toggle |
| `AlertDialog` had no way to dismiss without selecting a model | Added X `ft.IconButton(ft.Icons.CLOSE, on_click=lambda e: page.pop_dialog())` inside an `ft.Row` title; `app_state["model_id"]` unchanged on close |
| Model picker showed spinner every open even after first fetch | `_prefetch_models()` discarded response; replaced with `_preload_models()` that stores into `app_state["models_cache"]`; passed as `cached_models=` to picker — renders immediately on re-open |
